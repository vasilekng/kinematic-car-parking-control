...
Реализация двумерного симулятора автономной параллельной парковки транспортного средства задним ходом. Проект моделирует движение колесного робота с учетом неголономных ограничений, включает систему Collision Detection на основе матричных преобразований и реализует адаптивное управление на базе пропорционального регулятора (P-controller).

Проект разработан в рамках подготовки к поступлению на совместную образовательную программу **Yandex Physical AI Garage и МАИ** (Прикладная математика, 1 курс).
...
В основе физического движка лежит кинематическая модель велосипеда (Kinematic Bicycle Model), редуцирующая четырехколесное шасси до двухколесного эквивалента.
...
### Детекция столкновений (Линейная алгебра)
...
## Инженерные вызовы: Проблема заметаемого объема (Swept Volume)

В процессе интеграции Collision Detection с П-регулятором была выявлена классическая проблема робототехники. 
П-регулятор строит траекторию для *точечного центра масс* (задней оси). Однако реальный автомобиль имеет габариты. При сильном вывороте колес передний и задний бамперы описывают широкие дуги (заметаемый объем). Из-за этого при стартах вблизи линии парковки углы машины могли виртуально "цеплять" бордюр еще на этапе подхода.
**Решение:** На данном этапе зона детекции коллизий была строго ограничена физическими стенками самого кармана с небольшим допуском. Для полноценного Production-решения требуется переход от наивного П-регулятора к полноценному планировщику (см. пункт ниже).
...
## Ограничения и пути развития (Future Work)

Данная версия является концептом. Для создания полноценного стека автономной навигации планируется:
1. **Глобальное планирование пути (Path Planning):** П-регулятор не способен припарковать машину из сложных позиций, требующих маневров "вперед-назад". Планируется внедрение алгоритмов **Hybrid A\*** или **RRT**, которые строят траекторию с изначальным учетом габаритов (Bounding Box) и неголономных ограничений.
2. **Усовершенствование контроллера:** Замена П-регулятора на алгоритмы, специфичные для следования по траектории (Path Tracking): *Pure Pursuit* или *Stanley Controller*.
3. **Учет динамики:** Переход от кинематики к динамической модели (масса, силы трения, угол увода шин / Slip Angle).
4. **Улучшение интегратора:** Замена метода Эйлера на метод Рунге-Кутты 4-го порядка (RK4) для устранения накопления ошибки при расчетах.
